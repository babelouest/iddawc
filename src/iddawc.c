...
static int string_use_char_list_only(const char * str, const char * char_list) {
  ...
  if (!o_strnullempty(str) && !o_strnullempty(char_list)) {
...
static int _i_load_jwks_endpoint(struct _i_session * i_session) {
  ...
  if (i_session != NULL && json_string_length(json_object_get(i_session->openid_config, "jwks_uri"))) {
    ...
    ulfius_set_request_properties(&request, U_OPT_HEADER_PARAMETER, "Accept", "application/json",
...
static int _i_parse_redirect_to_parameters(struct _i_session * i_session, struct _u_map * map) {
  ...
  for (i=0; keys[i] != NULL; i++) {
    key = keys[i];
    if (0 == o_strcasecmp(key, "code") && (i_get_response_type(i_session) & I_RESPONSE_TYPE_CODE) && !o_strnullempty(u_map_get(map, key))) {
    ...
    } else if (0 == o_strcasecmp(key, "id_token") && i_get_response_type(i_session) & I_RESPONSE_TYPE_ID_TOKEN && !o_strnullempty(u_map_get(map, key))) {
      c_ret = i_set_str_parameter(i_session, I_OPT_ID_TOKEN, u_map_get(map, key));
      ret = ret!=I_OK?ret:c_ret;
    } else if (0 == o_strcasecmp(key, "access_token") && (i_get_response_type(i_session) & I_RESPONSE_TYPE_TOKEN) && !o_strnullempty(u_map_get(map, key))) {
      if (i_session->decrypt_access_token) {
        if ((payload_dup = _i_decrypt_jwe_token(i_session, u_map_get(map, key))) != NULL) {
          ...
          if (c_ret == I_OK) {
            if (r_jwt_token_type(i_get_str_parameter(i_session, I_OPT_ACCESS_TOKEN)) != R_JWT_TYPE_NONE &&
                i_verify_jwt_access_token(i_session, NULL) != I_OK) {
              y_log_message(Y_LOG_LEVEL_ERROR, "_i_parse_redirect_to_parameters - Error i_verify_jwt_access_token");
              c_ret = I_ERROR;
            }
          }
          ret = ret!=I_OK?ret:c_ret;
        ...
        ret = ret!=I_OK?ret:c_ret;
        if (o_strnullempty(u_map_get_case(map, "token_type"))) {
  ...
  if (i_session != NULL && r_jwks_size(i_session->client_jwks)) {
    ...
    if ((kid != NULL && (jwk_sign = r_jwks_get_by_kid(i_session->client_jwks, kid)) != NULL) ||
      ...
      if ((alg == R_JWA_ALG_RS256 || alg == R_JWA_ALG_RS384 || alg == R_JWA_ALG_RS512 ||
        ...
        if ((r_jwk_key_type(jwk_sign, NULL, i_session->x5u_flags) & (R_KEY_TYPE_RSA|R_KEY_TYPE_PRIVATE)) != (R_KEY_TYPE_RSA|R_KEY_TYPE_PRIVATE)) {
        ...
        if ((r_jwk_key_type(jwk_sign, NULL, i_session->x5u_flags) & (R_KEY_TYPE_EC|R_KEY_TYPE_PRIVATE)) != (R_KEY_TYPE_EC|R_KEY_TYPE_PRIVATE)) {
          y_log_message(Y_LOG_LEVEL_ERROR, "_i_get_dpop_sign_privkey - Invalid signing key type");
          r_jwk_free(jwk_sign);
          jwk_sign = NULL;
        }
      } else if (alg == R_JWA_ALG_EDDSA) {
        if ((r_jwk_key_type(jwk_sign, NULL, i_session->x5u_flags) & (R_KEY_TYPE_EDDSA|R_KEY_TYPE_PRIVATE)) != (R_KEY_TYPE_EDDSA|R_KEY_TYPE_PRIVATE)) {
          y_log_message(Y_LOG_LEVEL_ERROR, "_i_get_dpop_sign_privkey - Invalid signing key type");
          r_jwk_free(jwk_sign);
          jwk_sign = NULL;
        }
...
static char * _i_sign_encrypt_jwt_auth(struct _i_session * i_session, jwt_t * jwt) {
  ...
  if (i_session->auth_method & I_AUTH_METHOD_JWT_ENCRYPT_SECRET) {
    ...
    if ((i_session->server_kid != NULL && (jwk_enc = r_jwks_get_by_kid(i_session->server_jwks, i_session->server_kid)) != NULL) || (r_jwks_size(i_session->server_jwks) == 1 && (jwk_enc = r_jwks_get_at(i_session->server_jwks, 0)) != NULL)) {
      ...
      if ((enc_alg == R_JWA_ALG_RSA1_5 || enc_alg == R_JWA_ALG_RSA_OAEP || enc_alg == R_JWA_ALG_RSA_OAEP_256) && _i_has_openid_config_parameter_value(i_session, "request_object_encryption_alg_values_supported", r_jwa_alg_to_str(enc_alg))) {
        if ((r_jwk_key_type(jwk_enc, NULL, i_session->x5u_flags) & (R_KEY_TYPE_RSA|R_KEY_TYPE_PUBLIC)) != (R_KEY_TYPE_RSA|R_KEY_TYPE_PUBLIC)) {
        ...
        if ((r_jwk_key_type(jwk_enc, NULL, i_session->x5u_flags) & (R_KEY_TYPE_EC|R_KEY_TYPE_PUBLIC)) != (R_KEY_TYPE_EC|R_KEY_TYPE_PUBLIC)) {
          y_log_message(Y_LOG_LEVEL_ERROR, "Invalid encrypt key type");
          ret = I_ERROR_PARAM;
        }
      } else {
...
static char * _i_generate_auth_jwt(struct _i_session * i_session) {
  jwt_t * jwt = NULL;
  jwk_t * jwk_sign = NULL;
  char * jwt_str = NULL, * dpop_jkt = NULL;
  ...
  if (i_session != NULL) {
    ...
    if (i_session->use_dpop && (i_session->response_type & I_RESPONSE_TYPE_CODE)) {
      if ((jwk_sign = _i_get_dpop_sign_privkey(i_session)) != NULL && (dpop_jkt = r_jwk_thumbprint(jwk_sign, R_JWK_THUMB_SHA256, i_session->x5u_flags)) != NULL) {
        r_jwt_set_claim_str_value(jwt, "dpop_jkt", dpop_jkt);
      }
      o_free(dpop_jkt);
      r_jwk_free(jwk_sign);
      jwk_sign = NULL;
    ...
    if ((jwt_str = _i_sign_encrypt_jwt_auth(i_session, jwt)) == NULL) {
      y_log_message(Y_LOG_LEVEL_ERROR, "_i_generate_auth_jwt - Error _i_sign_encrypt_jwt_auth");
    }
...
static char * _i_generate_ciba_jwt(struct _i_session * i_session) {
  jwt_t * jwt = NULL, * jwt_login_hint = NULL;
  jwk_t * jwk_sign = NULL;
  char * jwt_str = NULL, * dpop_jkt = NULL, * tmp;
  const char ** keys = NULL;
  unsigned int i;
  ...
  if (i_session != NULL) {
    if ((i_session->auth_method & I_AUTH_METHOD_JWT_ENCRYPT_PUBKEY) && !(i_session->auth_method & (I_AUTH_METHOD_JWT_SIGN_PRIVKEY|I_AUTH_METHOD_JWT_SIGN_SECRET))) {
      y_log_message(Y_LOG_LEVEL_ERROR, "_i_generate_ciba_jwt - Error, signature required to authenticate the client");
      ret = I_ERROR_PARAM;
    } else {
      r_jwt_init(&jwt);
      r_jwt_set_claim_str_value(jwt, "client_id", i_session->client_id);
      ...
      if (i_session->ciba_login_hint_format == I_CIBA_LOGIN_HINT_FORMAT_ID_TOKEN) {
        r_jwt_set_claim_str_value(jwt, "id_token_hint", i_session->ciba_login_hint);
      }
      ...
      if (i_session->ciba_binding_message != NULL) {
        r_jwt_set_claim_str_value(jwt, "binding_message", i_session->ciba_binding_message);
      }
      if (i_session->ciba_acr_values != NULL) {
        r_jwt_set_claim_str_value(jwt, "acr_values", i_session->ciba_acr_values);
      }
      ...
      do {
        if (i_session->ciba_login_hint_format == I_CIBA_LOGIN_HINT_FORMAT_JSON) {
          if ((j_login_hint = json_loads(i_session->ciba_login_hint, JSON_DECODE_ANY, NULL)) != NULL) {
            ...
            r_jwt_set_claim_str_value(jwt, "login_hint", tmp);
            o_free(tmp);
          } else {
            y_log_message(Y_LOG_LEVEL_ERROR, "_i_generate_ciba_jwt - Error parsing login_hint");
            ret = I_ERROR_PARAM;
            break;
          }
        }
        ...
        if (i_session->ciba_login_hint_format == I_CIBA_LOGIN_HINT_FORMAT_JWT) {
          r_jwt_init(&jwt_login_hint);
          if (i_session->client_sign_alg != R_JWA_ALG_UNKNOWN) {
            sign_alg = i_session->client_sign_alg;
          } else if (i_session->ciba_request_signing_alg != R_JWA_ALG_UNKNOWN) {
            sign_alg = i_session->ciba_request_signing_alg;
          }
          if (r_jwt_set_full_claims_json_str(jwt_login_hint, i_session->ciba_login_hint) == RHN_OK) {
            if (i_session->token_method == I_TOKEN_AUTH_METHOD_JWT_SIGN_SECRET) {
              if (i_session->client_secret != NULL) {
                ...
                if (sign_alg != R_JWA_ALG_UNKNOWN) {
                  r_jwt_set_sign_alg(jwt_login_hint, sign_alg);
                  r_jwk_init(&jwk_sign);
                  r_jwk_import_from_symmetric_key(jwk_sign, (const unsigned char *)i_session->client_secret, o_strlen(i_session->client_secret));
                } else {
                  y_log_message(Y_LOG_LEVEL_ERROR, "Invalid signing key parameters");
            ...
            if (ret == I_OK) {
              tmp = r_jwt_serialize_signed(jwt_login_hint, jwk_sign, i_session->x5u_flags);
              r_jwt_set_claim_str_value(jwt, "login_hint_token", tmp);
              o_free(tmp);
            }
          } else {
      ...
      r_jwt_free(jwt_login_hint);
      r_jwk_free(jwk_sign);
      json_decref(j_login_hint);

      if ((jwt_str = _i_sign_encrypt_jwt_auth(i_session, jwt)) == NULL) {
        y_log_message(Y_LOG_LEVEL_ERROR, "i_run_ciba_auth_request - Error _i_sign_encrypt_jwt_auth");
      }
    ...
    if (i_session->token_method & I_TOKEN_AUTH_METHOD_JWT_ENCRYPT_SECRET) {
      ...
      if ((i_session->server_kid != NULL && (jwk_enc = r_jwks_get_by_kid(i_session->server_jwks, i_session->server_kid)) != NULL) || (r_jwks_size(i_session->server_jwks) == 1 && (jwk_enc = r_jwks_get_at(i_session->server_jwks, 0)) != NULL)) {
        ...
        if ((enc_alg == R_JWA_ALG_RSA1_5 ||
          ...
          if ((r_jwk_key_type(jwk_enc, NULL, i_session->x5u_flags) & (R_KEY_TYPE_RSA|R_KEY_TYPE_PRIVATE)) != (R_KEY_TYPE_RSA|R_KEY_TYPE_PRIVATE)) {
            y_log_message(Y_LOG_LEVEL_ERROR, "Invalid encrypt key type");
            ret = I_ERROR_PARAM;
          }
          ...
          if ((r_jwk_key_type(jwk_enc, NULL, i_session->x5u_flags) & (R_KEY_TYPE_EC|R_KEY_TYPE_PRIVATE)) != (R_KEY_TYPE_EC|R_KEY_TYPE_PRIVATE)) {
            y_log_message(Y_LOG_LEVEL_ERROR, "Invalid encrypt key type");
            ret = I_ERROR_PARAM;
          }
        } else {
...
int i_init_session(struct _i_session * i_session) {
  ...
  if (i_session != NULL) {
    ...
    i_session->dpop_nonce_as = NULL;
    i_session->dpop_nonce_rs = NULL;
    ...
    i_session->ciba_acr_values = NULL;
...
void i_clean_session(struct _i_session * i_session) {
  if (i_session != NULL) {
    ...
    o_free(i_session->ciba_acr_values);
  ...
  if (i_session != NULL) {
    switch (option) {
      ...
      case I_OPT_CIBA_CLIENT_NOTIFICATION_TOKEN:
        ...
        break;
      case I_OPT_CIBA_ACR_VALUES:
        o_free(i_session->ciba_acr_values);
        if (!o_strnullempty(s_value)) {
          i_session->ciba_acr_values = o_strdup(s_value);
        } else {
          i_session->ciba_acr_values = NULL;
        }
        break;
      case I_OPT_CIBA_ACR_VALUES_APPEND:
        if (!o_strnullempty(s_value)) {
          if (i_session->ciba_acr_values == NULL) {
            i_session->ciba_acr_values = o_strdup(s_value);
          } else {
            i_session->ciba_acr_values = mstrcatf(i_session->ciba_acr_values, " %s", s_value);
          }
        }
        break;
      ...
      case I_OPT_ID_TOKEN_SID:
        ...
        break;
      case I_OPT_DPOP_NONCE_AS:
        o_free(i_session->dpop_nonce_as);
        if (!o_strnullempty(s_value)) {
          i_session->dpop_nonce_as = o_strdup(s_value);
        } else {
          i_session->dpop_nonce_as = NULL;
        }
        break;
      case I_OPT_DPOP_NONCE_RS:
        o_free(i_session->dpop_nonce_rs);
        if (!o_strnullempty(s_value)) {
          i_session->dpop_nonce_rs = o_strdup(s_value);
        } else {
          i_session->dpop_nonce_rs = NULL;
        }
        break;
...
int i_add_claim_request(struct _i_session * i_session, int target, const char * claim, int essential, const char * value) {
  ...
  if (i_session != NULL && !o_strnullempty(claim)) {
...
int i_get_userinfo_custom(struct _i_session * i_session, const char * http_method, struct _u_map * additional_query, struct _u_map * additional_headers) {
  ...
  if (i_session != NULL && i_session->userinfo_endpoint != NULL && i_session->access_token != NULL) {
    ...
    if (ulfius_set_request_properties(&request, U_OPT_HEADER_PARAMETER, "Authorization", bearer, U_OPT_NONE) == U_OK) {
      ...
      if (_i_send_http_request(i_session, &request, &response) == U_OK) {
        if (i_session->use_dpop && u_map_get(response.map_header, "DPoP-Nonce") != NULL) {
          i_set_str_parameter(i_session, I_OPT_DPOP_NONCE_RS, u_map_get(response.map_header, "DPoP-Nonce"));
        }
...
int i_parse_redirect_to(struct _i_session * i_session) {
  ...
  if (o_strncmp(redirect_to, i_session->redirect_uri, o_strlen(i_session->redirect_uri)) == 0) {
    ...
    query = query_dup;
    if (!o_strnullempty(query) && query[0] == '?') {
      query++;
    }
    ...
    if (!o_strnullempty(query) && _i_has_openid_config_parameter_value(i_session, "response_modes_supported", "query")) {
      query_parsed = 1;
      u_map_init(&map);
  ...
  if (i_session != NULL) {
    switch (option) {
      ...
      case I_OPT_CIBA_CLIENT_NOTIFICATION_TOKEN:
        ...
        break;
      case I_OPT_CIBA_ACR_VALUES:
        result = (const char *)i_session->ciba_acr_values;
        break;
      case I_OPT_CIBA_AUTH_REQ_ID:
      ...
      case I_OPT_ID_TOKEN_SID:
        ...
        break;
      case I_OPT_DPOP_NONCE_AS:
        result = (const char *)i_session->dpop_nonce_as;
        break;
      case I_OPT_DPOP_NONCE_RS:
        result = (const char *)i_session->dpop_nonce_rs;
        break;
      default:
        break;
    }
...
int i_build_auth_url_get(struct _i_session * i_session) {
  int ret;
  char * url = NULL, * escaped = NULL, * tmp = NULL, * jwt = NULL, * dpop_jkt = NULL;
  const char ** keys = NULL;
  unsigned int i;
  ...
  if (i_session != NULL && i_session->client_id != NULL) {
    if (i_session->pushed_authorization_request_uri != NULL) {
      ...
      o_free(url);
    } else if (i_session->response_type != I_RESPONSE_TYPE_PASSWORD &&
      ...
      if (i_session->auth_method & (I_AUTH_METHOD_JWT_SIGN_SECRET|I_AUTH_METHOD_JWT_SIGN_PRIVKEY|I_AUTH_METHOD_JWT_ENCRYPT_SECRET|I_AUTH_METHOD_JWT_ENCRYPT_PUBKEY)) {
        ...
        if (i_session->use_dpop && (i_session->response_type & I_RESPONSE_TYPE_CODE)) {
          if ((jwk_sign = _i_get_dpop_sign_privkey(i_session)) != NULL && (dpop_jkt = r_jwk_thumbprint(jwk_sign, R_JWK_THUMB_SHA256, i_session->x5u_flags)) != NULL) {
            url = mstrcatf(url, "&dpop_jkt=%s", dpop_jkt);
          }
          o_free(dpop_jkt);
          r_jwk_free(jwk_sign);
        }
        ...
        for (i=0; keys[i] != NULL; i++) {
          if (!o_strnullempty(u_map_get(&i_session->additional_parameters, keys[i]))) {
...
int i_run_auth_request(struct _i_session * i_session) {
  ...
  if (i_session != NULL &&
    ...
    if (_i_init_request(i_session, &request) != U_OK || ulfius_init_response(&response) != U_OK) {
      ...
      if (ret == I_OK) {
        if (_i_send_http_request(i_session, &request, &response) == U_OK) {
          if (response.status == 302 && !o_strnullempty(u_map_get_case(response.map_header, "Location"))) {
      ...
      if (json_string_length(json_object_get(j_response, "access_token")) &&
        ...
        if (i_session->decrypt_access_token) {
          if ((payload_dup = _i_decrypt_jwe_token(i_session, json_string_value(json_object_get(j_response, "access_token")))) != NULL) {
            if ((res = i_set_str_parameter(i_session, I_OPT_ACCESS_TOKEN, payload_dup)) == I_OK) {
              if (r_jwt_token_type(i_get_str_parameter(i_session, I_OPT_ACCESS_TOKEN)) != R_JWT_TYPE_NONE &&
                  i_verify_jwt_access_token(i_session, NULL) != I_OK) {
                y_log_message(Y_LOG_LEVEL_ERROR, "i_parse_token_response - Error i_verify_jwt_access_token");
                ret = I_ERROR;
              }
            }
          } else {
    ...
    if (i_session->response_type & I_RESPONSE_TYPE_CODE) {
      if (i_session->redirect_uri != NULL &&
        ...
        if ((res = _i_add_token_authentication(i_session, i_session->token_endpoint, &request, sign_alg, enc_alg, enc)) == I_OK) {
          if (_i_send_http_request(i_session, &request, &response) == U_OK) {
            if (i_session->use_dpop && u_map_get(response.map_header, "DPoP-Nonce") != NULL) {
              i_set_str_parameter(i_session, I_OPT_DPOP_NONCE_AS, u_map_get(response.map_header, "DPoP-Nonce"));
            }
                ...
                if (response.status == 200 || response.status == 400) {
                  ...
                  if (j_response != NULL) {
                    if (i_parse_token_response(i_session, (int)response.status, j_response) == I_OK) {
                      ...
                      y_log_message(Y_LOG_LEVEL_ERROR, "i_run_token_request client_credentials - Error i_parse_token_response error");
                      ret = I_ERROR_PARAM;
                    }
                    json_decref(j_response);
                  } else {
...
int i_revoke_token(struct _i_session * i_session, int authentication) {
  ...
  if (i_session != NULL && !o_strnullempty(i_session->revocation_endpoint) && !o_strnullempty(i_session->token_target)) {
    if (_i_init_request(i_session, &request) != U_OK || ulfius_init_response(&response) != U_OK) {
      ...
      if (authentication == I_INTROSPECT_REVOKE_AUTH_ACCESS_TOKEN) {
        if (!o_strnullempty(i_session->access_token)) {
          // Set DPoP
          if (i_session->use_dpop) {
      ...
      if (!o_strnullempty(i_session->token_target_type_hint)) {
...
int i_get_token_introspection(struct _i_session * i_session, json_t ** j_result, int authentication, int get_jwt) {
  ...
  if (i_session != NULL && !o_strnullempty(i_session->introspection_endpoint) && !o_strnullempty(i_session->token_target) && j_result != NULL) {
    if (_i_init_request(i_session, &request) != U_OK || ulfius_init_response(&response) != U_OK) {
...
int i_register_client(struct _i_session * i_session, json_t * j_parameters, int update_session, json_t ** j_result) {
  ...
  if (i_session != NULL && !o_strnullempty(i_session->registration_endpoint)) {
...
int i_get_registration_client(struct _i_session * i_session, json_t ** j_result) {
  ...
  if (i_session != NULL && !o_strnullempty(i_session->registration_endpoint) && !o_strnullempty(i_session->client_id)) {
    if (_i_init_request(i_session, &request) != U_OK || ulfius_init_response(&response) != U_OK) {
...
int i_manage_registration_client(struct _i_session * i_session, json_t * j_parameters, int update_session, json_t ** j_result) {
  ...
  if (i_session != NULL && (!o_strnullempty(i_session->registration_client_uri) || (!o_strnullempty(i_session->registration_endpoint) && !o_strnullempty(i_session->client_id)))) {
    if (!json_is_array(json_object_get(j_copy_parameters, "redirect_uris"))) {
      json_object_set_new(j_copy_parameters, "redirect_uris", json_array());
    }
    ...
    if (_i_init_request(i_session, &request) != U_OK || ulfius_init_response(&response) != U_OK) {
      ...
      if (!o_strnullempty(i_session->registration_client_uri)) {
...
int i_import_session_json_t(struct _i_session * i_session, json_t * j_import) {
  ...
  if (i_session != NULL && json_is_object(j_import)) {
    if ((ret = i_set_parameter_list(i_session,
                                   ...
                                   I_OPT_CIBA_ACR_VALUES, json_string_value(json_object_get(j_import, "ciba_acr_values")),
...
int i_import_session_from_registration(struct _i_session * i_session, json_t * j_registration) {
  ...
  if (i_session != NULL && json_is_object(j_registration)) {
    ...
    ret = i_set_parameter_list(i_session, I_OPT_CLIENT_ID, json_string_value(json_object_get(j_registration, "client_id")),
                                          ...
                                          I_OPT_ACCESS_TOKEN_SIGNING_ALG, json_string_value(json_object_get(j_registration, "access_token_signing_alg")),
                                          I_OPT_ACCESS_TOKEN_ENCRYPTION_ALG, json_string_value(json_object_get(j_registration, "access_token_encryption_alg")),
                                          I_OPT_ACCESS_TOKEN_ENCRYPTION_ENC, json_string_value(json_object_get(j_registration, "access_token_encryption_end")),
                                          I_OPT_ID_TOKEN_SIGNING_ALG, json_string_value(json_object_get(j_registration, "id_token_signing_alg")),
                                          I_OPT_ID_TOKEN_ENCRYPTION_ALG, json_string_value(json_object_get(j_registration, "id_token_encryption_alg")),
                                          I_OPT_ID_TOKEN_ENCRYPTION_ENC, json_string_value(json_object_get(j_registration, "id_token_encryption_end")),
                                          I_OPT_USERINFO_SIGNING_ALG, json_string_value(json_object_get(j_registration, "userinfo_signing_alg")),
                                          I_OPT_USERINFO_ENCRYPTION_ALG, json_string_value(json_object_get(j_registration, "userinfo_encryption_alg")),
                                          I_OPT_USERINFO_ENCRYPTION_ENC, json_string_value(json_object_get(j_registration, "userinfo_encryption_end")),
                                          I_OPT_REQUEST_OBJECT_SIGNING_ALG, json_string_value(json_object_get(j_registration, "request_object_signing_alg")),
                                          I_OPT_REQUEST_OBJECT_ENCRYPTION_ALG, json_string_value(json_object_get(j_registration, "request_object_encryption_alg")),
                                          I_OPT_REQUEST_OBJECT_ENCRYPTION_ENC, json_string_value(json_object_get(j_registration, "request_object_encryption_end")),
                                          I_OPT_TOKEN_ENDPOINT_SIGNING_ALG, json_string_value(json_object_get(j_registration, "token_endpoint_signing_alg")),
                                          I_OPT_TOKEN_ENDPOINT_ENCRYPTION_ALG, json_string_value(json_object_get(j_registration, "token_endpoint_encryption_alg")),
                                          I_OPT_TOKEN_ENDPOINT_ENCRYPTION_ENC, json_string_value(json_object_get(j_registration, "token_endpoint_encryption_end")),
                                          I_OPT_CIBA_REQUEST_SIGNING_ALG, json_string_value(json_object_get(j_registration, "ciba_request_signing_alg")),
                                          I_OPT_CIBA_REQUEST_ENCRYPTION_ALG, json_string_value(json_object_get(j_registration, "ciba_request_encryption_alg")),
                                          I_OPT_CIBA_REQUEST_ENCRYPTION_ENC, json_string_value(json_object_get(j_registration, "ciba_request_encryption_end")),
...
char * i_generate_dpop_token(struct _i_session * i_session, const char * htm, const char * htu, time_t iat, int add_ath) {
  ...
  if (i_session != NULL && !o_strnullempty(i_session->token_jti) && !o_strnullempty(htu) && !o_strnullempty(htm)) {
    if (r_jwt_init(&jwt_dpop) == RHN_OK) {
      if ((jwk_sign = _i_get_dpop_sign_privkey(i_session)) != NULL) {
          ...
          if (r_jwk_extract_pubkey(jwk_sign, jwk_pub, i_session->x5u_flags) == RHN_OK) {
            if ((j_dpop_pub = r_jwk_export_to_json_t(jwk_pub)) != NULL) {
              if (add_ath) {
                ...
                if (i_get_str_parameter(i_session, I_OPT_DPOP_NONCE_RS) != NULL) {
                  if (r_jwt_set_claim_str_value(jwt_dpop, "nonce", i_get_str_parameter(i_session, I_OPT_DPOP_NONCE_RS)) != RHN_OK) {
                    y_log_message(Y_LOG_LEVEL_ERROR, "i_generate_dpop_token - access setting nonce");
                    has_error = 1;
                  }
                }
              } else {
                if (i_get_str_parameter(i_session, I_OPT_DPOP_NONCE_AS) != NULL) {
                  if (r_jwt_set_claim_str_value(jwt_dpop, "nonce", i_get_str_parameter(i_session, I_OPT_DPOP_NONCE_AS)) != RHN_OK) {
                    y_log_message(Y_LOG_LEVEL_ERROR, "i_generate_dpop_token - access setting nonce");
                    has_error = 1;
                  }
                }
          ...
          if (I_OK == ret) {
            ...
            if (_i_send_http_request(i_session, &copy_req, http_response) != U_OK) {
              ...
              if (u_map_get(http_response->map_header, "DPoP-Nonce") != NULL) {
                i_set_str_parameter(i_session, I_OPT_DPOP_NONCE_RS, u_map_get(http_response->map_header, "DPoP-Nonce"));
              }
            }
          }
        } else {
    ...
    if (r_jwt_advanced_parse(dpop_jwt, dpop_header, R_PARSE_HEADER_JWK, R_FLAG_IGNORE_REMOTE) == RHN_OK) {
      if (r_jwt_verify_signature(dpop_jwt, NULL, R_FLAG_IGNORE_REMOTE) == RHN_OK) {
        ...
        do {
          ...
          if (o_strnullempty(access_token)) {
...
int i_set_rich_authorization_request_str(struct _i_session * i_session, const char * type, const char * value) {
  ...
  if (i_session != NULL && !o_strnullempty(type) && !o_strnullempty(value)) {
    if ((j_value = json_loads(value, JSON_DECODE_ANY, NULL)) != NULL) {
...
int i_set_rich_authorization_request_json_t(struct _i_session * i_session, const char * type, json_t * j_value) {
  ...
  if (i_session != NULL && !o_strnullempty(type) && json_is_object(j_value)) {
...
int i_remove_rich_authorization_request(struct _i_session * i_session, const char * type) {
  ...
  if (i_session != NULL && !o_strnullempty(type)) {
    json_array_foreach(i_session->j_authorization_details, index, j_element) {
      if (0 == o_strcmp(type, json_string_value(json_object_get(j_element, "type")))) {
        json_array_remove(i_session->j_authorization_details, index);
        ret = I_OK;
...
int i_run_par_request(struct _i_session * i_session) {
  ...
  struct _u_response response;
  json_t * j_response;
  char * tmp, * redirect_to, * dpop_token;
  const char ** key = NULL;
  int i;
  jwa_alg sign_alg = R_JWA_ALG_UNKNOWN, enc_alg = R_JWA_ALG_UNKNOWN;
  jwa_enc enc = R_JWA_ENC_UNKNOWN;
  ...
  if (i_session != NULL &&
    ...
    if (i_session->use_dpop) {
      dpop_token = i_generate_dpop_token(i_session, "POST", _i_get_endpoint(i_session, "par"), 0, 0);
      if (ulfius_set_request_properties(&request, U_OPT_HEADER_PARAMETER, "DPoP", dpop_token, U_OPT_NONE) != U_OK) {
        y_log_message(Y_LOG_LEVEL_ERROR, "i_run_par_request code - Error setting DPoP header");
        ret = I_ERROR;
      }
      o_free(dpop_token);
    }
...
int i_run_ciba_request(struct _i_session * i_session) {
  ...
  if (i_session != NULL &&
    ...
    if (i_session->auth_method & (I_AUTH_METHOD_JWT_SIGN_SECRET|I_AUTH_METHOD_JWT_SIGN_PRIVKEY|I_AUTH_METHOD_JWT_ENCRYPT_SECRET|I_AUTH_METHOD_JWT_ENCRYPT_PUBKEY)) {
      if ((jwt_request = _i_generate_ciba_jwt(i_session)) != NULL) {
        ulfius_set_request_properties(&request, U_OPT_POST_BODY_PARAMETER, "request", jwt_request, U_OPT_NONE);
      } else {
        y_log_message(Y_LOG_LEVEL_ERROR, "i_run_ciba_request - Error _i_generate_ciba_jwt");
        ret = I_ERROR;
      }
      o_free(jwt_request);
    } else {
      ...
      if (i_session->ciba_acr_values != NULL) {
        ulfius_set_request_properties(&request, U_OPT_POST_BODY_PARAMETER, "acr_values", i_session->ciba_acr_values, U_OPT_NONE);
      }
